...
const webPort=Number(process.env.TINGLAN_WEB_PORT || 4318), bridgePort=Number(process.env.TINGLAN_BRIDGE_PORT || 4319);
if(![webPort,bridgePort].every(port=>Number.isInteger(port)&&port>1024&&port<65536)||webPort===bridgePort)throw new Error('本机端口配置无效');
const bridge=createBridge({port:bridgePort,allowedPorts:[webPort,bridgePort]});
...
const web=createServer((req,res)=>{
  ...
  if(![`127.0.0.1:${webPort}`,`localhost:${webPort}`].includes(host)){res.writeHead(403).end('Local access only');return;}
  let path;try{path=decodeURIComponent(new URL(req.url,`http://127.0.0.1:${webPort}`).pathname);}catch{res.writeHead(400).end();return;}
  res.setHeader('X-Content-Type-Options','nosniff');
  res.setHeader('Referrer-Policy','no-referrer');
  if(path.startsWith('/api/')){
    const upstream=request({hostname:'127.0.0.1',port:bridgePort,path:req.url,method:req.method,headers:{...req.headers,host:`127.0.0.1:${bridgePort}`}},response=>{
  ...
  if(path==='/__health'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'}).end(JSON.stringify({ok:true,...release,root,webPort,bridgePort,pid:process.pid}));return;}
...
await bridge.listen();
web.listen(webPort,'127.0.0.1',()=>console.log('Tinglan '+release.version+' '+release.commit+' at http://127.0.0.1:'+webPort+'/'));
